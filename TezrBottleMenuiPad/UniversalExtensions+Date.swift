...
extension String {
    ...
    init(timeInterval: TimeInterval, options: TimeIntervalOptions) {
        ...
        // If no units were added to string, add the smallest unit with the quantity
        //of zero
        if string == "" {
...
extension Date {
    ...
    func formattedStringWith(_ formatter: DateFormatterToken...) -> String {
        return self.formattedStringWith(formatter)
    }
...
extension Array where Element == DateFormatterToken {
    func reduceIntoString() -> String {
        return self.reduce("", { (result, token) -> String in
            return result + token.format
        })
    }
}
...
extension Date {
    ...
    /**
     ...
     - parameter components: which components are updated to the given `date`
     */
    func equating(to dateComponents: DateComponents, by components: Set<Calendar.Component>) -> Date {
        var selfComponents = DateComponents(date: self, forComponents: DateComponents.AllComponents)
